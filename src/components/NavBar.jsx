...
import { selectCartItemCount, clearCartLocal } from "../context/slice/cartSlice";
...
const NavBar = () => {
  ...
  const logoutHandler = ()=>{
    dispatch(logout());
    dispatch(clearCartLocal()); // Clear cart from Redux on logout
    navigate('/');
  }
    ...
    <NavLink to='/'> <img src={assets.logo} alt="logo-img" className="w-36"></img></NavLink>
     ...
     <div className="flex items-center gap-6">
        ...
        <div className="group relative">
          ...
          {user ? <div className="group-hover:block hidden absolute dropdown-menu right-0 pt-4">
            <div className="flex flex-col gap-2 w-36 py-3 px-5 bg-slate-100 text-gray-700 rounded-md shadow-lg">
              <NavLink to="/profile"><p className="cursor-pointer hover:text-black py-1">My Profile</p></NavLink>
              <NavLink to="/orders"><p className="cursor-pointer hover:text-black py-1">Orders</p></NavLink>
              <p onClick={logoutHandler} className="cursor-pointer hover:text-black py-1">Logout</p>
            </div>
          </div> : null}
      ...
      <Link to="/cart" className="relative">
       ...
       {cartItemCount > 0 && (
         <p className="absolute right-[-5px] bottom-[-5px] w-4 text-center leading-4 bg-black text-white aspect-square rounded-full text-[8px]">{cartItemCount}</p>
       )}
      </Link>
